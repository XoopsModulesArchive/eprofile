...
if (!defined('XOOPS_MAINFILE_INCLUDED')) {
    ...
    $xoopsTpl->assign('section_name', _PROFILE_MA_PMNAME);
} else {
    chdir( XOOPS_ROOT_PATH . '/modules/profile/' );
    xoops_loadLanguage('main', 'profile');
}
...
if (isset($_POST['op']) && $_POST['op'] == "submit") {
            ...
            if ($toUser->notify_method() == 2) {
              ...
              $xoopsMailer->assign('X_SITENAME', $GLOBALS['xoopsConfig']['sitename']);
     		  $xoopsMailer->assign('X_SITEURL', XOOPS_URL."/");
   		      ...
   		      $xoopsMailer->setSubject(_PROFILE_MA_PMMAILNOTIFYSUBJECT);
   		      $xoopsMailer->send(); 
            }            
    ...
    if ($reply == 1) {
        $pm_handler =& xoops_getModuleHandler('message', 'profile');
        $pm =& $pm_handler->get($msg_id);
        if ($pm->getVar("to_userid") == $xoopsUser->getVar('uid')) {
            $pm_uname = XoopsUser::getUnameFromId($pm->getVar("from_userid"));
            $message  = "[quote]\n";
            $message .= sprintf(_PROFILE_MA_PMUSERWROTE , $pm_uname);
    ...
    $pmform->addElement(new XoopsFormButton('', 'reset', _PROFILE_MA_PMCLEAR, 'reset'));
    
    $cancel_send = new XoopsFormButton('', 'cancel', _PROFILE_MA_PMCANCELSEND, 'button');
