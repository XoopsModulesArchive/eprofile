...
function profile_getFieldForm(&$field, $action = false)
{
    ...
    $title = $field->isNew() ? sprintf(_EPROFILE_AM_ADD, _EPROFILE_AM_FIELD) : sprintf(_EPROFILE_AM_EDIT, _EPROFILE_AM_FIELD);

    include_once(XOOPS_ROOT_PATH."/class/xoopsformloader.php");
    $form = new XoopsThemeForm($title, 'form', $action, 'post', true);

    $form->addElement(new XoopsFormText(_EPROFILE_AM_TITLE, 'field_title', 35, 255, $field->getVar('field_title', 'e')));
    $form->addElement(new XoopsFormTextArea(_EPROFILE_AM_DESCRIPTION, 'field_description', $field->getVar('field_description', 'e')));
    ...
    $cat_select = new XoopsFormSelect(_EPROFILE_AM_CATEGORY, 'field_category', $fieldcat_id);
    $cat_select->addOption(0, _EPROFILE_AM_DEFAULT);
    $cat_select->addOptionArray($category_handler->getList());
    $form->addElement($cat_select);
    $form->addElement(new XoopsFormText(_EPROFILE_AM_WEIGHT, 'field_weight', 10, 10, $field->getVar('field_weight', 'e')));
    if ($field->getVar('field_config') || $field->isNew()) {
        if (!$field->isNew()) {
            $form->addElement(new XoopsFormLabel(_EPROFILE_AM_NAME, $field->getVar('field_name')));
            $form->addElement(new XoopsFormHidden('id', $field->getVar('field_id')));
        } else {
            $form->addElement(new XoopsFormText(_EPROFILE_AM_NAME, 'field_name', 35, 255, $field->getVar('field_name', 'e')));
        }

        //autotext and theme left out of this one as fields of that type should never be changed (valid assumption, I think)
        $fieldtypes = array(
            'checkbox' => _EPROFILE_AM_CHECKBOX,
        ...
        $element_select = new XoopsFormSelect(_EPROFILE_AM_TYPE, 'field_type', $field->getVar('field_type', 'e'));
        ...
        switch ($field->getVar('field_type')) {
            case "textbox":
                $valuetypes = array(
                    XOBJ_DTYPE_ARRAY    => _EPROFILE_AM_ARRAY,
                ...
                $type_select = new XoopsFormSelect(_EPROFILE_AM_VALUETYPE, 'field_valuetype', $field->getVar('field_valuetype', 'e'));
        ...
        //$form->addElement(new XoopsFormRadioYN(_EPROFILE_AM_NOTNULL, 'field_notnull', $field->getVar('field_notnull', 'e')));
        ...
        if ($field->getVar('field_type') == "select" || $field->getVar('field_type') == "select_multi" || $field->getVar('field_type') == "radio" || $field->getVar('field_type') == "checkbox") {
            ...
            if (count($options) > 0) {
                $remove_options = new XoopsFormCheckBox(_EPROFILE_AM_REMOVEOPTIONS, 'removeOptions');
            ...
            $option_text = "<table  cellspacing='1'><tr><td width='20%'>" . _EPROFILE_AM_KEY . "</td><td>" . _EPROFILE_AM_VALUE . "</td></tr>";
            for ($i = 0; $i < 3; $i ++) {
            ...
            $option_text .= "</table>";
            $form->addElement(new XoopsFormLabel(_EPROFILE_AM_ADDOPTION, $option_text));
        }
    }
    ...
    if ($field->getVar('field_edit')) {
        switch ($field->getVar('field_type')) {
            ...
            case "textarea":
            case "dhtml":
                $form->addElement(new XoopsFormText(_EPROFILE_AM_MAXLENGTH, 'field_maxlength', 35, 35, $field->getVar('field_maxlength', 'e')));
                $form->addElement(new XoopsFormTextArea(_EPROFILE_AM_DEFAULT, 'field_default', $field->getVar('field_default', 'e')));
                break;

            case "checkbox":
            case "select_multi":
                $def_value = $field->getVar('field_default', 'e') != null ? unserialize($field->getVar('field_default', 'n')) : null;
                $element = new XoopsFormSelect(_EPROFILE_AM_DEFAULT, 'field_default', $def_value, 8, true);
                $options = $field->getVar('field_options');
                asort($options);
            ...
            case "radio":
                ...
                $element = new XoopsFormSelect(_EPROFILE_AM_DEFAULT, 'field_default', $def_value);
                $options = $field->getVar('field_options');
                asort($options);
                // If options do not include an empty element, then add a blank option to prevent any default selection
                if (!in_array('', array_keys($options))) {
            ...
            case "date":
                $form->addElement(new XoopsFormTextDateSelect(_EPROFILE_AM_DEFAULT, 'field_default', 15, $field->getVar('field_default', 'e')));
                break;

            case "longdate":
                $form->addElement(new XoopsFormTextDateSelect(_EPROFILE_AM_DEFAULT, 'field_default', 15, strtotime($field->getVar('field_default', 'e'))));
                break;

            case "datetime":
                $form->addElement(new XoopsFormDateTime(_EPROFILE_AM_DEFAULT, 'field_default', 15, $field->getVar('field_default', 'e')));
                break;

            case "yesno":
                $form->addElement(new XoopsFormRadioYN(_EPROFILE_AM_DEFAULT, 'field_default', $field->getVar('field_default', 'e')));
                break;

            case "timezone":
                $form->addElement(new XoopsFormSelectTimezone(_EPROFILE_AM_DEFAULT, 'field_default', $field->getVar('field_default', 'e')));
                break;

            case "language":
                $form->addElement(new XoopsFormSelectLang(_EPROFILE_AM_DEFAULT, 'field_default', $field->getVar('field_default', 'e')));
                break;

            case "group":
                $form->addElement(new XoopsFormSelectGroup(_EPROFILE_AM_DEFAULT, 'field_default', true, $field->getVar('field_default', 'e')));
                break;

            case "group_multi":
                $form->addElement(new XoopsFormSelectGroup(_EPROFILE_AM_DEFAULT, 'field_default', true, $field->getVar('field_default', 'e'), 5, true));
                break;

            case "theme":
                $form->addElement(new XoopsFormSelectTheme(_EPROFILE_AM_DEFAULT, 'field_default', $field->getVar('field_default', 'e')));
                break;
    ...
    if (in_array($field->getVar('field_type'), $searchable_types)) {
        ...
        $form->addElement(new XoopsFormSelectGroup(_EPROFILE_AM_PROF_SEARCH, 'profile_search', true, $search_groups, 5, true));
    }
    if ($field->getVar('field_edit') || $field->isNew()) {
        ...
        $form->addElement(new XoopsFormSelectGroup(_EPROFILE_AM_PROF_EDITABLE, 'profile_edit', false, $editable_groups, 5, true));
        $form->addElement(new XoopsFormRadioYN(_EPROFILE_AM_REQUIRED, 'field_required', $field->getVar('field_required', 'e')));
        $regstep_select = new XoopsFormSelect(_EPROFILE_AM_PROF_REGISTER, 'step_id', $field->getVar('step_id', 'e'));
...
function profile_getRegisterForm(&$user, $profile, $step = null)
{
    ...
    if ($step_no == 1) {
        //$uname_size = $xoopsConfigUser['maxuname'] < 35 ? $xoopsConfigUser['maxuname'] : 35;

        $elements[0][] = array('element' => new XoopsFormText(_US_NICKNAME, "uname", 35, $xoopsConfigUser['maxuname'], $user->getVar('uname', 'e')), 'required' => true);
        $weights[0][] = 0;
    ...
    foreach (array_keys($elements) as $k) {
        ...
        //$title = isset($categories[$k]) ? $categories[$k]['cat_title'] : _EPROFILE_MA_DEFAULT;
    ...
    if ($step_no == 1 && $xoopsConfigUser['reg_dispdsclmr'] != 0 && $xoopsConfigUser['reg_disclaimer'] != '') {
        ...
        $disc_text = new XoopsFormLabel("", "<div style=\"padding: 5px; overflow:auto; max-height: 100px;\">".$GLOBALS["myts"]->displayTarea($xoopsConfigUser['reg_disclaimer'],1)."</div>");
...
{
    global $xoopsConfig, $xoopsModule, $xoopsConfigUser, $xoopsUser;
    if ($action === false) {
        $action = $_SERVER['REQUEST_URI'];
    }
    if (empty($xoopsConfigUser)) {
    ...
    $title = $user->isNew() ? _EPROFILE_AM_ADDUSER : _US_EDITPROFILE;
    ...
    if ($user->isNew() || $xoopsUser->isAdmin()) {
        $elements[0][] = array('element' => new XoopsFormText(_US_NICKNAME, 'uname', 25, $xoopsConfigUser['maxuname'], $user->getVar('uname', 'e')), 'required' => 1);
        $email_text = new XoopsFormText('', 'email', 30, 60, $user->getVar('email'));
    } else {
        $elements[0][] = array('element' => new XoopsFormLabel('', $user->getVar('uname')), 'required' => 0);
        $email_text = new XoopsFormLabel('', $user->getVar('email'));
    }
    $email_tray = new XoopsFormElementTray(_US_EMAIL, '<br />');
    $email_tray->addElement($email_text, ($user->isNew() || $xoopsUser->isAdmin()) ? 1 : 0);
    $weights[0][] = 0;
    ...
    if ($xoopsUser->isAdmin() && $user->getVar('uid') != $xoopsUser->getVar('uid')) {
        ...
        $level_radio = new XoopsFormRadio(_EPROFILE_MA_USERLEVEL, 'level', $user->getVar('level'));
        $level_radio->addOption(1, _EPROFILE_MA_ACTIVE);
        $level_radio->addOption(0, _EPROFILE_MA_INACTIVE);
        //$level_radio->addOption(-1, _EPROFILE_MA_DISABLED);
    ...
    $all_categories = $cat_handler->getObjects(null, true, false);
	  $all_categories[0] = array('cat_id' => '0', 'cat_title' => _EPROFILE_MA_DEFAULT, 'cat_description' => '', 'cat_weight' => '0');
    ...
    if ($xoopsUser && $xoopsUser->isAdmin()) {
        xoops_loadLanguage('admin', 'eprofile');
        $gperm_handler =& xoops_gethandler('groupperm');
...
function profile_getStepForm($step = null, $action=false)
{
    ...
    $form = new XoopsThemeForm(_EPROFILE_AM_STEP, 'stepform', 'admin_step.php', 'post', true);

    if (!$step->isNew()) {
        $form->addElement(new XoopsFormHidden('id', $step->getVar('step_id')));
    }
    $form->addElement(new XoopsFormHidden('op', 'save'));
    $form->addElement(new XoopsFormText(_EPROFILE_AM_STEPNAME, 'step_name', 25, 255, $step->getVar('step_name', 'e')));
    $form->addElement(new XoopsFormText(_EPROFILE_AM_STEPINTRO, 'step_desc', 25, 255, $step->getVar('step_desc', 'e')));
    $form->addElement(new XoopsFormText(_EPROFILE_AM_STEPORDER, 'step_order', 10, 10, $step->getVar('step_order', 'e')));
    $form->addElement(new XoopsFormRadioYN(_EPROFILE_AM_STEPSAVE, 'step_save', $step->getVar('step_save', 'e')));
    $form->addElement(new XoopsFormButton('', 'submit', _SUBMIT, 'submit'));
