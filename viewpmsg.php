...
$GLOBALS['xoopsTpl']->assign('section_name', _PROFILE_MA_PMNAME);
$xoopsOption['xoops_pagetitle'] = sprintf(_US_ALLABOUT, $GLOBALS['xoopsUser']->getVar('uname'))." :: "._PROFILE_MA_PMNAME;
...
if (isset($_POST['delete_messages']) && isset($_POST['msg_id'])) {
    if (!$GLOBALS['xoopsSecurity']->check()) {
        ...
        xoops_confirm(array('ok' => 1, 'delete_messages' => 1, 'op' => $_REQUEST['op'], 'msg_id'=> serialize(array_map("intval", $_POST['msg_id']))), $_SERVER['REQUEST_URI'], _PROFILE_MA_PMRUSUREDELETE);
        ...
        $xoopsTpl->assign('msg', _PROFILE_MA_PMDELETED);
    }
}
if (isset($_POST['move_messages']) && isset($_POST['msg_id'])) {
    if (!$GLOBALS['xoopsSecurity']->check()) {
        ...
        if ($_POST['op'] == 'save') {
            $xoopsTpl->assign('msg', _PROFILE_MA_PMUNSAVED);
        } elseif (isset($total_save) && !$xoopsUser->isAdmin()) {
            $xoopsTpl->assign('msg', sprintf(_PROFILE_MA_PMSAVED_PART, $xoopsModuleConfig['max_save'], $i));
        } else {
            $xoopsTpl->assign('msg', _PROFILE_MA_PMSAVED_ALL);
        }
    }
}
if (isset($_REQUEST['empty_messages'])) {
    if (!$GLOBALS['xoopsSecurity']->check()) {
        ...
        xoops_confirm(array('ok' => 1, 'empty_messages' => 1, 'op' => $_REQUEST['op']), $_SERVER['REQUEST_URI'], _PROFILE_MA_PMRUSUREEMPTY);
        include XOOPS_ROOT_PATH."/footer.php";
        exit();
    } else {
        if ($_POST['op'] == 'save') {
        ...
        $xoopsTpl->assign('msg', _PROFILE_MA_PMEMPTIED);
    }
}
...
$send_button = new XoopsFormButton('', 'send', _PROFILE_MA_PMSEND);
$send_button->setExtra("onclick='javascript:openWithSelfMain(\"" . XOOPS_URL . "/modules/profile/pmlite.php?send=1\", \"pmlite\", 550, 450);'");
$delete_button = new XoopsFormButton('', 'delete_messages', _PROFILE_MA_PMDELETE, 'submit');
$move_button = new XoopsFormButton('', 'move_messages', ($_REQUEST['op'] == 'save') ? _PROFILE_MA_PMUNSAVE : _PROFILE_MA_PMTOSAVE, 'submit');
$empty_button = new XoopsFormButton('', 'empty_messages', _PROFILE_MA_PMEMPTY, 'submit');
