...
switch($op) {
    ...
    case "list":
        ...
        $form = new XoopsThemeForm(_EPROFILE_AM_EDITUSER, 'form', 'admin_user.php');
        $form->addElement(new XoopsFormSelectUser(_EPROFILE_AM_SELECTUSER, 'id'));
    ...
    case "edit":
        ...
        if (in_array(XOOPS_GROUP_ADMIN, $obj->getGroups()) && !in_array(XOOPS_GROUP_ADMIN, $xoopsUser->getGroups())) {
            ...
            redirect_header("admin_user.php", 3, _US_NOEDITRIGHT);
			exit();
        }
        include_once '../include/forms.php';
        $form = profile_getUserForm($obj);
    ...
    case "save":
        ...
        if (!$GLOBALS['xoopsSecurity']->check()) {
            redirect_header('admin_user.php', 3, _US_NOEDITRIGHT . "<br />" . implode('<br />', $GLOBALS['xoopsSecurity']->getErrors()));
            exit();
        }
        ...
        if (count($errors) == 0) {
            if ($handler->insertUser($user)) {
                ...
                if ($user->isNew()) {
                    redirect_header('admin_user.php', 2, _EPROFILE_AM_USERCREATED, false);
                } else {
                    redirect_header('admin_user.php', 2, _US_PROFUPDATED, false);
                }
				exit();
            }
        } else {
    ...
    case "delete":
        if ($_REQUEST['id'] == $xoopsUser->getVar('uid')) {
            redirect_header('admin_user.php', 2, _EPROFILE_AM_CANNOTDELETESELF);
			exit();
        }
        $obj =& $handler->getUser($_REQUEST['id']);
        $groups = $obj->getGroups();
        if (in_array(XOOPS_GROUP_ADMIN, $groups)) {
            redirect_header('admin_user.php', 3, _EPROFILE_AM_CANNOTDELETEADMIN, false);
			exit();
        }
        
        if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
            if (!$GLOBALS['xoopsSecurity']->check()) {
                redirect_header('admin_user.php', 3, implode(',', $GLOBALS['xoopsSecurity']->getErrors()), false);
				exit();
            }
            $profile_handler = xoops_getmodulehandler('profile');
            ...
            if (!$profile || $profile->isNew() || $profile_handler->delete($profile)) {
                if ($handler->deleteUser($obj)) {
                    redirect_header('admin_user.php', 3, sprintf(_EPROFILE_AM_DELETEDSUCCESS, $obj->getVar('uname') . " (" . $obj->getVar('email') . ")"), false);
					exit();
                } else {
            ...
            xoops_confirm(array('ok' => 1, 'id' => $_REQUEST['id'], 'op' => 'delete'), $_SERVER['REQUEST_URI'], sprintf(_EPROFILE_AM_RUSUREDEL, $obj->getVar('uname') . " (" . $obj->getVar('email') . ")"));
        }
        break;
}
